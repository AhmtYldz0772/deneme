 ...
 */
void SX1272OnDio0Irq( nrf_drv_gpiote_pin_t pin, nrf_gpiote_polarity_t action );

/*!
 * \brief DIO 1 IRQ callback
 */
void SX1272OnDio1Irq( nrf_drv_gpiote_pin_t pin, nrf_gpiote_polarity_t action );

/*!
 * \brief DIO 2 IRQ callback
 */
void SX1272OnDio2Irq( nrf_drv_gpiote_pin_t pin, nrf_gpiote_polarity_t action );

/*!
 * \brief DIO 3 IRQ callback
 */
void SX1272OnDio3Irq( nrf_drv_gpiote_pin_t pin, nrf_gpiote_polarity_t action );

/*!
 * \brief DIO 4 IRQ callback
 */
void SX1272OnDio4Irq( nrf_drv_gpiote_pin_t pin, nrf_gpiote_polarity_t action );

/*!
 * \brief DIO 5 IRQ callback
 */
void SX1272OnDio5Irq( nrf_drv_gpiote_pin_t pin, nrf_gpiote_polarity_t action );
...
void SX1272Init( RadioEvents_t *events )
{
    ...
    {
        SX1272SetModem( RadioRegsInit[i].Modem );
        readValue=SX1272Read( RadioRegsInit[i].Addr);
    }    
...
void SX1272WriteBuffer( uint16_t addr, uint8_t *buffer, uint8_t size )
{
    ...
    SpiInOut( &SX1272.Spi, addr | 0x80, 1 );
	
		
    for( i = 0; i < size; i++ )
    {
        SpiInOut( &SX1272.Spi, buffer[i], 1 );
    }
...
void SX1272ReadBuffer( uint16_t addr, uint8_t *buffer, uint8_t size )
{
    ...
    SpiInOut( &SX1272.Spi, addr & 0x7F, 1 );

    for( i = 0; i < size; i++ )
    {
        buffer[i] = SpiInOut( &SX1272.Spi, 0, 0 );
    }
    //NSS = 1;
    GpioWrite( &SX1272.Spi.Nss, 1 );
