...
extern float32_t sensorData[X_Y_AXIS_NUM][FFT_BUFFER_SIZE];
...
void vInitFIFOtoEeprom (void)
{
  ...
  if(pageControl > 0 && pageControl < 6)
  {
    ...
    memcpy(&writingData[2], &sensorData[0][0], pageControl);
    ...
    memcpy(&writingData[2], &sensorData[pageControl][pageControl], (6 - pageControl) );
    while(nrf_drv_twi_is_busy(&m_twi));
  ...
  else
  {
    ...
    writingData[1] = (uint8_t)eepromWriteAddress;
    writingData[0] = (uint8_t)(eepromWriteAddress >> 8) ; 
    memcpy(&writingData[2], &sensorData[0][0], 6);
...
void prvSendEepromData( uint16_t bluetoothCounter)
{
