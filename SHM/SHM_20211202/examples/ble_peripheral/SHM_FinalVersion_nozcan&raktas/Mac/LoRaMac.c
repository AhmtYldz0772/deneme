 ...
 */
static bool SrvAckRequested = true;
...
static void OnRadioRxDone( uint8_t *payload, uint16_t size, int16_t rssi, int8_t snr )
{
    ...
    //printf("macHdr.Valu %d:\n",macHdr.Value);
    //printf("macHdr.Value is %d:\n", macHdr.Bits.Major);
    //printf("macHdr.Value is %d:\n", macHdr.Bits.RFU);
    printf("Type is %d:\r\n",macHdr.Bits.MType  );
    ...
    switch( mibSet->Type )
    {
        ...
        case MIB_ANTENNA_GAIN:
        {
            ...
            break;
				...
				case MIB_MULTICAST_CHANNEL:
				{
            if( mibSet->Param.MulticastList != NULL )
            {
							LoRaMacMulticastChannelLink(mibSet->Param.MulticastList);
                //memcpy1( LoRaMacAppSKey, mibSet->Param.AppSKey,
                  //             sizeof( LoRaMacAppSKey ) );
            }
            else
