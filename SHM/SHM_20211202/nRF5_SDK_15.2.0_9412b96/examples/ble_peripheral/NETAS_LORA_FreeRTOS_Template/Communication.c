...
void SPIRead(uint8_t address, uint8_t regSize)
{
  ...
  switch  (regSize)
  {
    ...
    case SPI_READ_ONE_REG:
      APP_ERROR_CHECK(nrf_drv_spi_transfer(&spi2, SPITxBuf, 1, (uint8_t *)&rxData2, 2));  
    break;
    case SPI_READ_TWO_REG:
        APP_ERROR_CHECK(nrf_drv_spi_transfer(&spi2, SPITxBuf, SPI_SEND_ONE_REG, SPIRxBuf, SPI_READ_TWO_REG));
    break;                                                                                              
    case SPI_READ_THREE_REG:
      APP_ERROR_CHECK(nrf_drv_spi_transfer(&spi2, SPITxBuf, SPI_SEND_ONE_REG, SPIRxBuf, SPI_READ_THREE_REG));
    break;
